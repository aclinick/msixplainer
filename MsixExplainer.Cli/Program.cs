...
static class Program
{
    ...
    static void PrintConsoleReport(PackageInfo info, List<ManifestFinding> findings)
    {
        ...
        var table = new Table()
            ...
            .Title("[bold]MSIX Manifest Explainer[/]")
        ...
        // Footer
        AnsiConsole.Write(new Rule($"[grey]Generated by MSIX Manifest Explainer — {DateTime.Now:yyyy-MM-dd HH:mm}[/]")
    ...
    {
        AnsiConsole.Write(new FigletText("MSIX Explainer").Color(Color.CornflowerBlue));
        ...
        table.AddRow("  msixexplainer [grey]<file.msix>[/]", "Analyze a package");
        table.AddRow("  msixexplainer [grey]*.msix[/]", "Analyze multiple packages");
        table.AddRow("  msixexplainer [grey]--sample[/]", "Analyze built-in sample manifest");
        table.AddRow("", "");
        ...
        table.AddRow("  msixexplainer app.msix", "Full console report");
        table.AddRow("  msixexplainer app.msix --json -o report.json", "Save JSON report");
        table.AddRow("  msixexplainer *.msix --severity warning", "Only warnings+critical");
        table.AddRow("  msixexplainer app.msix -q && echo PASS", "CI gate check");
